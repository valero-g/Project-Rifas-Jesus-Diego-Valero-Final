...
def add_vendedor():
    try:
        ...
        vendedor = db.session.execute(select(Vendedor).where(Vendedor.nombre == request_request_body["nombre"])).scalar_one_or_none()
        if vendedor is not None:
            return {"message": "El vendedor ya existe"}, 400

        # Agregamos el vendedor a la tabla
        new_vendedor = Vendedor(nombre=request_request_body["nombre"], direccion=request_request_body["direccion"], telefono=request_request_body["telefono"], email=request_request_body["email"], persona_contacto = request_request_body["persona_contacto"])
...
def add_rifa():
    try:
        ...
        new_rifa = Rifas(nombre_rifa=request_request_body["nombre_rifa"], vendedor_id=request_request_body["vendedor_id"],fecha_de_sorteo=request_request_body["fecha_de_sorteo"],hora_de_sorteo=request_request_body["hora_de_sorteo"],precio_boleto=request_request_body["precio_boleto"],premio_rifa=request_request_body["premio_rifa"],url_premio=request_request_body["url_premio"],numero_max_boletos=request_request_body["numero_max_boletos"],status_sorteo=request_request_body["status_sorteo"], boleto_ganador=request_request_body["boleto_ganador"])
...
def add_usuario():
    try:
        ...
        user_email = db.session.execute(select(Usuario).where(Usuario.email == request_body["email"])).scalar_one_or_none()
        user_usuario = db.session.execute(select(Usuario).where(Usuario.usuario == request_body["usuario"])).scalar_one_or_none()
        ...
        new_user = Usuario(usuario = request_body["usuario"], email = request_body["email"], contraseña =  hash.decode('utf-8'), nombre = request_body["nombre"], apellidos = request_body ["apellidos"], direccion_envio = request_body["direccion_envio"], dni = request_body["dni"], telefono = request_body["telefono"], stripe_customer_id = "0", status = False)
        ...
        msg = Message(subject='Confirma tu email para registrate en 4Boleeks', sender='info4boleeks@yahoo.com', recipients=[new_user.email])
...
def login():
    try:
        ...
        user_mail = db.session.execute(select(Usuario).where(Usuario.email == email)).scalar_one_or_none()
        user_usuario = db.session.execute(select(Usuario).where(Usuario.usuario == usuario)).scalar_one_or_none()
